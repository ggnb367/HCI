...
class Graph:
    ...
    def from_triples(
        ...
        *,
        ...
        return cls._build_from_triple_iter(
            triples,
    ...
    def from_knowledge_graph(
        ...
        *,
        ...
        try:
            ...
            raise ValueError("文件第一行需要是三元组数量，例如 151442") from exc

        triple_count = 0
        progress: Optional[ProgressTracker]
        progress = ProgressTracker(expected, "读取三元组") if expected > 0 else None

        def triple_generator() -> Iterator[Tuple[object, object, object]]:
            nonlocal triple_count, progress
            try:
                with open(triple_path, "r", encoding="utf8") as fh_inner:
                    fh_inner.readline()  # skip header
                    for line in fh_inner:
                        line = line.strip()
                        if not line:
                            continue
                        parts = line.split()
        ...
        graph = cls._build_from_triple_iter(
            triple_generator(),
            assume_undirected=assume_undirected,
            weight_strategy=weight_strategy,
        )

        if expected != 0 and triple_count != expected:
...
def dijkstra_to_targets(
    ...
    """Run Dijkstra but only keep distances to nodes inside ``targets``."""
    ...
    if source in targets and source not in distances:
...
class GTreeIndex:
    ...
    def build(
        ...
        *,
        leaf_size: int = 64,
        fanout: int = 4,
        precompute_boundaries: bool = False,
    ) -> "GTreeIndex":
            ...
            def precompute(node: GTreeNode) -> None:
                if node.boundary:
                    ...
                    for source in node.boundary:
                        distances = dijkstra_to_targets(
                            self_graph,
                            source,
                            node.boundary,
                            restrict_to=node.vertices,
                        )
...
def cmd_build(args: argparse.Namespace) -> None:
    ...
    index = GTreeIndex.build(
        ...
        fanout=args.fanout,
        precompute_boundaries=args.precompute_boundaries,
    )
